...
from Networks import DQN_Img_Net
...
Net = DQN_Img_Net(input_size=args.input_size, num_actions=args.num_actions)
Memory = DQN_Memory(10000)
...
def main():
    ...
    for i_ep in range(10000):
        ...
        if i_ep % 250 == 0 and i_ep != 0:
            ...
            i = str(i_ep / 250)
            agent.save_param()
            np.savetxt('rewards_double_dqn.txt', rewards)
            np.savetxt('avd_reward_double_dqn.txt', avg_rewards)
            plt.plot(rewards)
