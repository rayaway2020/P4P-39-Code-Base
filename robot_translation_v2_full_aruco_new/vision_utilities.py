...
full_path = os.path.join(home, 'workspace/test_bed_RL_robot/robot_translation_v2_full_aruco_new/camera_calibration')
...
class Vision:
    def __init__(self):
        ...
        self.markerSize = 0.018  # size of the aruco marker meters
    ...
    def check_same_contents(self, list1, list2):
        p = []
        for i in list1:
            p.append(i[0])
        for x in set(p + list2):
            if p.count(x) != list2.count(x):
                return False
        return True

    def calculate_marker_pose(self, target_in_pixel_x, target_in_pixel_y):
        ...
        try:
            if self.check_same_contents(IDs, self.robot_marks_id):
                ...
                for id_marker in self.robot_marks_id:
                    ...
                    if index is None:
                        self.vision_flag_status = False
                        return 0, image, self.vision_flag_status, None, None
                    if id_marker == 0:
                ...
                target_location = np.array(target_point_wrt_camera)
                target_position = target_location * 100
                # np.array(target_point_wrt_camera)

                state_space = (joint_0_arm_1_location * 100, joint_0_arm_2_location * 100,
                               joint_1_arm_1_location * 100, joint_1_arm_2_location * 100,
                               end_arm_1_location * 100, end_arm_2_location * 100,
                               cube_location * 100, target_location * 100)

                state_space = [element for state_space_list in state_space for element in state_space_list]

                self.vision_flag_status = True
                return np.array(state_space), image, self.vision_flag_status, cube_in_cm, target_position
    ...
    def plot_state_space(self, observation_space_vector):
        ...
        cv2.rectangle(img, (target[0], target[1]), (target[0] + size_rectangle, target[1] + size_rectangle),
                      (0, 0, 255), -1)
    ...
    def normalization_conversion(self, point, imag_h, imag_w):
        ...
        position_x = (point[0] - (-22)) * (imag_w - 0) / (22 - (-22)) # 180
        position_y = (point[1] - (-16)) * (imag_h - 0) / (16 - (-16)) # 120
