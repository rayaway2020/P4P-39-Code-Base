...
from Networks import ActorTD3, CriticTD3
...
class TD3agent_rotation:
    ...
    def save_model(self):
        torch.save(self.actor.state_dict(), 'models/td3_actor_cylinder.pth')
        torch.save(self.critic_q1.state_dict(), 'models/td3_critic_1_cylinder.pth')
        torch.save(self.critic_q2.state_dict(), 'models/td3_critic_2_cylinder.pth')
...
def main_run():
    ...
    env = RL_ENV()
    agent = TD3agent_rotation(env, batch_size=batch_size)
    ...
    for episode in range(1, num_episodes + 1):
        ...
        for step in range(1, episode_horizont + 1):
            ...
            cylinder_angle = next_state[-1]
            reward, done = env.calculate_reward(cylinder_angle)
            agent.add_experience_memory(state, action, reward, next_state, done)
            episode_reward += reward

            env.env_render(image_state, done, step, episode, cylinder_angle)
    ...
    np.savetxt('result/rewards.txt', rewards)
    np.savetxt('result/avd_reward.txt', avg_rewards)
